...
local Colors = {
    ...
    function KukuriLib:CreateWindow(title, subtitle)
        ...
        window.Tabs = {}
            ...
            -- SciFiIndicator: Will act as the left border for the active tab
            local sciFiIndicator = Instance.new("Frame")
            sciFiIndicator.Name = "SciFiIndicator"
            sciFiIndicator.Size = UDim2.new(0, 4, 1, 0) -- Full height, 4px wide
            sciFiIndicator.AnchorPoint = Vector2.new(0, 0.5)
            sciFiIndicator.Position = UDim2.new(0, -4, 0.5, 0) -- Initially hidden by being off-screen left
            sciFiIndicator.BackgroundColor3 = Colors.DarkPrimary -- Color for active state
            ...
            tabIcon.Position = UDim2.new(0, 15, 0.5, 0) -- Positioned after where the indicator might be
            ...
            bottomBorder.ZIndex = tabButton.ZIndex -- Below icon/text/indicator
            ...
            local function selectThisTab()
                for _, t_info in pairs(self.Tabs) do
                    ...
                    -- Hide indicator for inactive tabs
                    t_info.indicator.BackgroundColor3 = Colors.DarkPrimary -- Keep color consistent even if hidden
                    TweenService:Create(t_info.indicator, tweenInfoMedium, {
                        Position = UDim2.new(0, -t_info.indicator.Size.X.Offset, 0.5, 0),
                        Size = UDim2.new(0, 4, 1, 0) -- Ensure correct size when hiding
                    }):Play()
                ...
                -- Style for the CURRENTLY SELECTED tab (to match image)
                tabContent.Visible = true
                TweenService:Create(tabButton, tweenInfoFast, {BackgroundColor3 = Colors.MainBackground}):Play() -- Lighter background
                TweenService:Create(tabLabel, tweenInfoFast, {TextColor3 = Colors.DarkPrimary}):Play()       -- Darker text
                TweenService:Create(tabIcon, tweenInfoFast, {BackgroundColor3 = Colors.DarkPrimary}):Play()         -- Darker icon
                
                -- Configure and show indicator for active tab
                sciFiIndicator.BackgroundColor3 = Colors.DarkPrimary -- Dark indicator
                TweenService:Create(sciFiIndicator, tweenInfoMedium, {
                    Position = UDim2.new(0, 0, 0.5, 0), -- Flush left
                    Size = UDim2.new(0, 4, 1, 0)          -- Ensure full height
                }):Play()

                if bottomBorder then
                    bottomBorder.Visible = false -- No bottom border for active tab
                end
            ...
            tabButton.MouseButton1Click:Connect(function()
                if self.CurrentTabContent ~= tabContent then -- Only run if not already selected
                    selectThisTab()
                end
            end)
            ...
            tabButton.MouseEnter:Connect(function()
                if self.CurrentTabContent ~= tabContent then -- Only if tab is NOT active
            ...
            -- Initial setup
            if #self.Tabs == 1 then -- If this is the first tab, select it
                selectThisTab()
            else -- Otherwise, style as an unselected tab
                tabButton.BackgroundColor3 = Colors.UnselectedTabBackground
                tabLabel.TextColor3 = Colors.UnselectedTabText
                tabIcon.BackgroundColor3 = Colors.UnselectedTabIcon
                sciFiIndicator.BackgroundColor3 = Colors.DarkPrimary -- Set default color for consistency
                sciFiIndicator.Size = UDim2.new(0, 4, 1, 0)          -- Ensure correct size from start
                sciFiIndicator.Position = UDim2.new(0, -sciFiIndicator.Size.X.Offset, 0.5, 0) -- Hidden
                if bottomBorder then
                    bottomBorder.BackgroundColor3 = Colors.TabSeparatorLine
            ...
            function tab:CreateParagraph(text)
                ...
                coroutine.wrap(function()
                    ...
                    if availableWidth == 0 and self.UI.Parent.AbsoluteSize.X > 0 then
                        if self.UI.Parent and self.UI.Parent.Parent and self.UI.Parent.Parent:FindFirstChild("ContentLayout") then
                            local parentContentLayout = self.UI.Parent.Parent:FindFirstChild("ContentLayout")
                            if parentContentLayout and parentContentLayout:IsA("UIListLayout") then
                                availableWidth = self.UI.Parent.AbsoluteSize.X - (parentContentLayout.Padding.Offset * 2)
                            else
                                availableWidth = self.UI.Parent.AbsoluteSize.X
                            end
                        elseif self.UI.Parent then
                             availableWidth = self.UI.Parent.AbsoluteSize.X
                        end

                        local localLayout = self.UI:FindFirstChildOfClass("UIListLayout")
                        if localLayout then
                             availableWidth = self.UI.AbsoluteSize.X
                             if availableWidth == 0 and self.UI.Parent then
                                availableWidth = self.UI.Parent.AbsoluteSize.X - 20
                             end
                        end
        ...
        function tab:CreateSlider(text, min, max, default, callback)
            ...
            local animationUpdateIntervalFrames = 15 -- high = slow
            print("KukuriLib Slider:", animationUpdateIntervalFrames)
            local animationFrameCounter = 0
